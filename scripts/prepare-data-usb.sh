...
# Optional: copy build-manifest.json if present.
#
# CONTRACT (producer side): generated by scripts/generate-build-records.sh
# as the final step of run-update.sh; consumed by show-status on the
# scanning host (via load-scanner, which copies it off the data USB).
# Optional because a refresh from a pre-manifest state may not have one
# yet; preserving the ability to write a data USB without it keeps
# recovery paths open. show-status treats absence by reporting "unknown"
# for every build-identity field — see its CONTRACT note. Keep these two
# messages consistent.
if [ -f "${SIGDIR}/build-manifest.json" ]; then
    ...
    echo "    The data USB will work without it; show-status will report 'unknown'"
    echo "    for build identity."
fi
