...
- **Upstream artefact integrity verification.** Every software component the build station fetches is integrity-verified at download. Verification paths differ by what each upstream publishes; the project records the verification method per component so an auditor can see at a glance how trust in each is established. The verification taxonomy is:
  ...
  | `tls-only` | YARA Forge | YARA Forge publishes no sidecars or signatures alongside its release assets. TLS-from-GitHub is the only integrity guarantee at download time. The downloaded archive's SHA-256 is recorded per-build (planned, see the per-build manifest roadmap) so a scan can be reproduced even though no upstream value exists to verify against. |
...
- **YARA Forge publishes no integrity sidecars.** Unlike Firecracker and LOKI-RS, YARA Forge release assets carry neither `.sha256` files nor GPG signatures. TLS-from-GitHub is the only integrity guarantee at download time. The downloaded archive's hash is recorded for reproducibility but did not gate the download. Mitigation in place is operational (the per-build manifest preserves what was used), not preventive.
- **Floating upstream channels.** `COREOS_INSTALLER_TAG="release"` and `COREOS_STREAM="stable"` remain moving targets. Pinning them is the planned next supply-chain hardening step but is tracked separately: pinning the CoreOS stream requires accepting a different bump workflow (security updates flow through the stream, so pinning means accepting responsibility for tracking CVEs); pinning the installer container by digest requires a different mechanism (`docker pull` by digest rather than SHA-256 of a downloaded file).
