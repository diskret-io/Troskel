...
- **Nation-state zero-days** in the block layer, virtio-blk, ext4, or Firecracker's hypervisor surface. The architecture limits consequences but cannot prevent the breach.
...
- **Signature freshness gate.** `check-system-ready` fails if ClamAV signatures are >30 days old. (YARA Forge rule freshness is recorded under `/var/lib/troskel/yara-rules-date` but not currently enforced — separate from ClamAV-signature freshness in cadence and provenance.)
...
- **YARA-rule freshness not gated.** `download-yara-rules.sh` records a refresh date, but `check-system-ready` only enforces ClamAV-signature age. A scanner image built with stale rules would pass the readiness check.
